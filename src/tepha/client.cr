module Tepha
  class Client
    ...
    private def message_id(activity) : String
      ...
      response = Halite.get(message_url, headers: {"Authorization" => ["Bearer", Constants::WEBEX_TEAMS_ACCESS_TOKEN].join(" ")})
    ...
    def run : Void
      ...
      socket.on_open do
        message = {
          "id"         => UUID.random.to_s,
          "type"       => "authorization",
          "trackingId" => ["tepha", "-", UUID.random.to_s].join(""),
          "data"       => {
            "token" => ["Bearer", Constants::WEBEX_TEAMS_ACCESS_TOKEN].join(" "),
          },
        }
